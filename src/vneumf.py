...
class VNeuMF(torch.nn.Module):
    def __init__(self, config):
        ...
        self.fc_embedding = torch.nn.Linear(in_features=2048, out_features=self.latent_dim_v)
        ...
        for idx, (in_size, out_size) in enumerate(zip(config['layers_v'][:-1], config['layers_v'][1:])):
            ...
            self.fc_layers_v.append(torch.nn.Dropout(0.1))
    ...
    def forward(self, user_indices, item_indices, poster_embeddings):
        ...
        item_embedding_v = self.fc_embedding(poster_embeddings)
